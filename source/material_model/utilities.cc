...
namespace aspect
{
  namespace MaterialModel
  {
    namespace MaterialUtilities
    {
      ...
      std::vector<double>
      compute_only_composition_fractions(
        ...
        const double min_fraction)
      {
        ...
        for (unsigned int i = 0; i < indices_to_use.size(); ++i)
          {
            x_comp[i] =
              std::min(std::max(compositional_fields[indices_to_use[i]], 0.0), 1.0);

            if (scheme == CompositionFractionScheme::thresholded &&
                x_comp[i] < min_fraction)
              x_comp[i] = 1e-5;
        ...
        // Background fraction
        composition_fractions[0] =
          (sum_composition >= 1.0 ? 0.0 : 1.0 - sum_composition);
                                    ...
                                    const double min_fraction)
      {
        ...
        // Compute background field fraction
        composition_fractions[0] = (sum_composition >= 1.0 ? 0.0 : 1.0 - sum_composition);
