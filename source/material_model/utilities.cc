...
namespace aspect
{
  namespace MaterialModel
  {
    namespace MaterialUtilities
    {
      ...
      template <int dim>
      void
      compute_drucker_prager_yielding (const DruckerPragerInputs &in, DruckerPragerOutputs &out)
      {
        // plasticity
...
#define INSTANTIATE(dim) \
  ...
  template void compute_drucker_prager_yielding<2>(
    const DruckerPragerInputs &,
    DruckerPragerOutputs &
  );

  template void compute_drucker_prager_yielding<3>(
    const DruckerPragerInputs &,
    DruckerPragerOutputs &
  );
