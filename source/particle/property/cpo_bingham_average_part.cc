...
#include <aspect/particle/property/cpo_bingham_average_part.h>
...
namespace aspect
{
  namespace Particle
  {
    ...
    {
      template <int dim>
      CpoBinghamAverage_part<dim>::CpoBinghamAverage_part ()
      {}
      ...
      template <int dim>
      void
      CpoBinghamAverage_part<dim>::initialize ()
      {
      ...
      template <int dim>
      void
      CpoBinghamAverage_part<dim>::initialize_one_particle_property(const Point<dim> &,
      ...
      template <int dim>
      void
      CpoBinghamAverage_part<dim>::update_one_particle_property(const unsigned int data_position,
                                                           const Point<dim> &,
                                                           const Vector<double> &,
      ...
      CpoBinghamAverage_part<dim>::compute_bingham_average(std::vector<Tensor<2,3>> matrices) const
      {
      ...
      template <int dim>
      UpdateTimeFlags
      CpoBinghamAverage_part<dim>::need_update() const
      {
      ...
      template <int dim>
      UpdateFlags
      CpoBinghamAverage_part<dim>::get_needed_update_flags () const
      {
      ...
      CpoBinghamAverage_part<dim>::get_property_information() const
      {
      ...
      template <int dim>
      void
      CpoBinghamAverage_part<dim>::declare_parameters (ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Particles");
          {
            prm.enter_subsection("CPO Bingham Average part");
            {
      ...
      template <int dim>
      void
      CpoBinghamAverage_part<dim>::parse_parameters (ParameterHandler &prm)
      {
...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
      ASPECT_REGISTER_PARTICLE_PROPERTY(CpoBinghamAverage_part,
                                        "cpo bingham average part",
