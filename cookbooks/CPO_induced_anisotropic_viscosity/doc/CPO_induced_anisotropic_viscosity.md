...
\sigma_{ij} = \frac{1}{\gamma J(\sigma_{pq})^{(n-1)/2}} A_{ik}^{-1}\dot\varepsilon_{kj} \text{ .}
```
...
\sigma_{ij} = \frac{1}{\gamma J(R'_{pk}\sigma_{kl}R_{lq})^{(n-1)/2}}([R_K]_{im}A_{ml}^{-1}[R_K']_{lk}) \varepsilon_{kj} \text{ .}
```
...
\end{aligned}
```
with the anisotropic tensor for viscosity in $V_{ij}$ and the non-linear viscosity $\eta(\dot{\varepsilon}_I)$, which is based on an anisotropic strain-rate invariant $\dot{\varepsilon}_I$:
```{math}
